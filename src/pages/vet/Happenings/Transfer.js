...
const Transfer = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if(response.ok) {
        navigate('/vet/manage-pets/record-life-event', { state: { successMessage: 'Η μεταβίβαση καταχωρήθηκε με επιτυχία!' } });
        setLoading(false);
        setPet(
          { ...pet, 
            ownerId: formData.newOwnerId,
        ...
        navigate('/vet/manage-pets/record-life-event', { state: { errorMessage: 'Σφάλμα κατά την καταχώρηση της μεταβίβασης. Παρακαλώ δοκιμάστε ξανά.' } });
        setLoading(false);
      }
    } catch (error) {
  ...
  return (
    <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 2, alignSelf: 'center' }}>
        Mεταβίβαση Κατοικιδίου
      </Typography>
      <Typography variant="body1" sx={{ alignSelf: 'center' }}>
        Το ζώο μεταβιβάζεται από έναν ιδιώτη σε έναν άλλον ιδιώτη που θα είναι ο/η νέος/α του ιδιοκτήτης/τρια
      </Typography>
      <Box sx ={{ display: 'flex', flexDirection: 'column', ml: 4 }}>
        <Typography variant="h6" sx={{ fontWeight: 'bold', mt: 4}}>
          1. Κατοικίδιο προς μεταβίβαση:
        </Typography>
